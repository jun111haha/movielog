...
const TvPopularContainer = observer(() => {
  ...
  const getLoadData = debounce(() => {
    if (isIntersect && datatFinish === false) {
      tvListStore.tvPopularPage > 6
        ? setDatatFinish(true)
        : tvListStore.getPopularList(tvListStore.tvPopularPage++);
    }
  ...
  return (
    <>
      <TvPresenterComponent
        tv={
          <>
            {tvListStore.tvPopularList.map((data, index) => {
              return (
                <Poster
                  key={index}
                  id={data.id}
...
export default TvPopularContainer;
