...
const MovieUpcomingContainer = observer(() => {
  ...
  const getLoadData = debounce(() => {
    if (isIntersect && datatFinish === false) {
      movieListStore.movieUpcomingPage > 6
        ? setDatatFinish(true)
        : movieListStore.getMovieUpcomingList(
            movieListStore.movieUpcomingPage++
          );
    }
  ...
  return (
    <>
      <MoviePresenterComponent
        movie={
          <>
            {movieListStore.movieUpcomingList.map((data, index) => {
              return (
                <Poster
                  id={data.id}
                  key={index}
